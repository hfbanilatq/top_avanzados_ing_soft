...
test('Create and validate new team', async ({ page }) => {
  const teamData = new TeamData(
    ...
    teamDataJson.tier,
    teamDataJson.pokemon.map((p: any) => new Pokemon(
      p.name,
      p.item,
      p.moves,
      new EVStats(p.evs.hp, p.evs.atk, p.evs.def, p.evs.spa, p.evs.spd, p.evs.spe)
    ))
  );

  const homePage = new HomePage(page);
  const teamCreationPage = new TeamCreationPage(page, teamData.generation, teamData.tier);
